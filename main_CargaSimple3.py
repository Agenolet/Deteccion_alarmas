...
#pot_sum_f0 es la suma de potencia de las f0 de cada tono
#pot_armonicos es la suma de potencia de los armonicos de cada tono

pot_senial = pot_sum_f0
pot_ruido = np.sum(senial_UTI_fft_pot) - (pot_sum_f0 + pot_armonicos) #ME ESTA DANDO UN VALOR NEGATIVO, LA POT DE F0 Y ARMON ES MAYOR

# Calculo relacion señal-ruido (snr: signal-to-noise ratio)
snr = 10 * np.log10(pot_senial / pot_ruido)

print(f"La Relación Señal Ruido es de {snr:.2f} dB")
...
ax3[0].plot(t_UTI, senial_UTI, label='Señal de audio-ruidosa', zorder=1, color='blue')
...
ax3[1].set_ylabel('Magnitud [V/√Hz]', fontsize=15)
ax3[1].set_title('Espect potencia frec ruidosa', fontsize=15)
